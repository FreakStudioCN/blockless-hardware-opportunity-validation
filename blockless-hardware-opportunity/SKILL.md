...
9. 写作只从 `relevance-review.json` 每条记录的完整 `body` 取事实，每句“我们看见”标 `[n]`，先存内部版；再派一个没参与写作的核对者逐条对照，直到 STRETCH 0 / WRONG 0 才剥掉编号生成发送版。读取 [引用核对](references/citation-review.md)，这一步不可跳过。

## 强制研究门禁（不可跳过）

当用户只输入“产品名 + 描述”时，仍按以下顺序执行；不得直接写邮件正文、报告或泛品类检索。

1. 在 `output/<slug>/research-plan.json` 保存产品名、原始描述，以及 `user_scenarios`、`desired_outcomes`、`current_alternatives`、`product_form`、`engineering_constraints`、`commercial_constraints` 六组问题空间。每组至少一条；未知必须明确写作未知。同一文件保存 `core_terms`：至少 3 个离开它们就没法描述这个创意的英文词（如药盒创意的 `pill`、`medication`、`dispenser`），相关性审核用它们剔除只碰巧共享两个泛词的帖子。
2. 在同一文件中生成至少 8 条英文定向检索词，覆盖至少 4 个问题空间维度。每条必须保存 `dimension`、`query`、`why`；其中至少一半是用户时刻、结果或替代方案词，不能全部是产品类别、品牌或技术词。每条 2–3 个实词最好，超过 5 个门禁拒绝：公开搜索接口按短语匹配，整句检索什么都搜不到。
3. 用这些检索词运行公开采集和聚类。Reddit 默认不请求，仅在明确需要且端点可访问时附加 `--include-reddit`；它不能是主要证据来源。优先使用 Hacker News 的帖子与评论、GitHub、Stack Exchange、允许访问的官方社区、产品评论和一手来源。若 Reddit 或另一来源被拒绝，保留错误日志，并补充允许访问的公开来源；不可把失败来源计作覆盖。Stack Exchange 站点用 `--stackexchange-sites` 按创意选（默认 electronics、diy、superuser、parenting、workplace、psychology），选这个创意的用户真正提问的社区。
4. 运行 `scripts/review_relevance.py --case-dir .\\output\\<slug>` 执行 Codex 自动相关性审核，并写入 `evidence/relevance-review.json`：至少 30 条、最多 40 条与目标任务有关的记录，每条保留 `url`、`task_link`、`signal`、原始查询、实际查询变体、评分、发布日期和完整 `body`。自动审核按任务词与实际检索词的交集初筛，再要求标题或正文整词命中至少一个 `core_terms`，同一 URL 只保留一条；无关的新闻摘要、爬虫镜像、代码日报、纯技术 issue 必须排除。审核后翻一遍保留记录的标题，七八成无关就回去改 `core_terms` 和检索词，不得把词法相关性写成付费或采用证据。
5. 运行：
...
命令不是 `RESEARCH GATE: PASSED` 时，只能交付“证据不足/采集受阻”的内部说明，不能生成或更新客户邮件正文、PDF、评分或结论。通过后必须同时输出标准客户邮件 Markdown、人味版客户邮件 Markdown、共创路线图 Markdown、完整客户报告 Markdown 和客户报告 PDF。顺序固定：四份带 `[n]` 的内部版写入 `evidence/internal-with-citations/` → 独立核对者把结果写入 `evidence/citation-review.md`，按清单修改直到最新一轮 STRETCH 0 / WRONG 0（见 [引用核对](references/citation-review.md)）→ `scripts/strip_citations.py --case-dir .\\output\\<slug>` 生成发送版 → `scripts/render_client_pdf.py` 生成 PDF 时指定 `--preview evidence/pdf-render-preview.png` → `scripts/validate_client_delivery.py --case-dir .\\output\\<slug>`。交付校验会核对每个 `[n]` 都指向已接受记录、发送版与内部版剥编号后逐字相同、发送版没有研究黑话、核对结果干净且晚于内部版最后一次修改；PDF 生成、PNG 渲染预览和交付校验任何一项失败都不得交付。邮件中的每一个“我们看见”的断言都必须能回链到通过审阅的记录或一手来源。
